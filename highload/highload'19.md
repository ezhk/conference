...
В TW передается job-spec с информацией о распределении по регионам и ДЦ. Основные компоненты:

- front (API) с состоянием
- Scheduler proxy — распределяет запросы на обновление контейнеров
- Sheduler — координирует работу нескольких контейнеров
- resource allocator — «где запустить контейнеры?», аллокатор отвечает на вопрос «что?», а шедулер — «как?»
- Resource broker — предоставляет унифицированную картину ресурсов и серверов, помогает аллокаторам работать между собой и является хранилищем данных аллокаторов
- TW agent

*В канждом регионе работает TW независимо.*  
TW vs Kubernetes:

- K проще расширять
- K более user friendly
- K лучше поддерживает persistent volumens
- K лучше поддерживает GPU и дисковые ресурсы
- K позволяет подуключать модули, например, для лучшего сбора логов
- T — масштабируемость, у K начинаются проблем, когда больше 5k nod или 300k контейнеров
- T — лучше поддержка stateful сервисов
- T — лучше поддержка fault tolerance
- T — лучше устроен capacity management
- T — лучше поддержка maintenance и failure handling
- для T единый runtime

### Resource broker

- унификация представления серверов для сервисов
- поддержка инфраструктуры ДЦ, координация работы внутри ДЦ — представление картины для ДЦ
- автоматическая ребалансировка нагрузки (перерапределяет контейнеры, если нехватает ресурсов)
- умеет off pick, чтобы доливать нагрузку на простаивающие серверы

RB не должен ни от чего зависеть. Не должен использовать инфраструктурные компонениы — свой storage layer.  
Данные должны быть строго консистентны.  
В качестве хранилища используется Delos (внутренняя разработка).  

Delos:

- rocks DB таблицы
- shared log (ранее был zookeeper), сейчас всё на диске