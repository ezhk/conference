...
Fix it, labels identify resources.<br />
-> _rs.yaml_:
...
foo-r7lgs stay, because has other label.<br />
...
$ curl 127.0.0.1:8001/api/v1 # list of API objects<br />
...
Replicaset extension:<br />
$ curl 127.0.0.1:8001/apis/extensions/v1beta<br />
...
^^^ It's internal call, throught API:<br />
$ curl 127.0.0.1:8001/api/v1/namespaces/default/pods<br />
Or for pod:<br />
$ kubectl get pods
...
$ curl 127.0.0.1:8001/api/v1/namespaces/default/pods/foo-r7lgs<br />
...
Namespace is a scope for the names/resources.<br />
$ kubectl create -f pod.yaml
...
Create pod in quota namespace:<br />
-> modify _pod.yaml_:
...
Remove namespace in file:<br />
$ kubectl create -f pod.yaml

    pod "foo" created

We can use network policy between different namespaces.<br />
Detailed about namespaces: https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/<br />
...
Try this https://github.com/kubernetes/examples/blob/master/guestbook/all-in-one/guestbook-all-in-one.yaml<br />
$ kubectl get service
...
Abstractions stay at one place:<br />
$ kubectl get svc
...
Look at it: localhost:8001/api/v1/proxy/namespaces/default/services/game/<br />
$ kubectl get svc game -o yaml | grep -i link

  selfLink: /api/v1/namespaces/default/services/game

How can we do this w/o proxy:<br />
...
Now we've got new image, how to update?<br />
...
1777248768 — hash by image version.<br />
After 5 minutes docker stop trying.<br />
...
Each revision - each replicaset.<br />
...
Also read about Canary release, as kubernetes do this:<br />
https://hackernoon.com/canary-release-with-kubernetes-1b732f2832ac<br />
https://kubernetes.io/docs/concepts/cluster-administration/manage-deployment/#canary-deployments<br />
...
deploy and rollover do w/ controller.<br />

**It's old concept, use deployments!**

# Create own WordPress
$ kubectl run mysql --image=mysql:5.5 --env=MYSQL_ROOT_PASSWORD=root<br />
...
$ kubectl exec -ti mysql-1883654754-0126r -- mysql -p -uroot<br />
...
# Ingress Controller
It's an nginx proxy, driven by kubernets API:<br />
dynamically reconfigure controller w/ nginx.<br />
Continiously monitored endpoints.<br />
...
Simple example:<br />
https://github.com/sebgoa/oreilly-kubernetes/blob/master/manifests/ingress-controller/wordpress.yaml<br />

$ curl 192.168.99.100

    default backend - 404

But wordpress.192.168.99.100.nip.io works!<br />

Check scaling:<br />
$ kubectl get pods -n wordpress<br />
...
*It's make simplify to work between pods, like a localhost.*<br />
https://kubernetes.io/docs/tasks/access-application-cluster/communicate-containers-same-pod-shared-volume/

# Basic schemes of Day 1 discuss
Pic.1 - summary scheme of architecture namespaces.<br />
Pic.2 — Pod scheme.<br />
...
$ kubectl exec -ti nginx-4217019353-t8rt0 -- /bin/bash<br />
root@nginx-4217019353-t8rt0:/# ls -la /var/run/secrets/kubernetes.io/serviceaccount/<br />

*Volume shared space between containers.*<br />
emptyDir and hostPath volumes are extremely easy to use (and understand). emptyDir is an empty directory that gets erased when the Pod dies (but survives container restarts), HostPath volumes survive Pod deletion.<br />

Example:<br />
https://github.com/sebgoa/oreilly-kubernetes/tree/master/manifests/volumes/volumes.yaml<br />
$ kubectl create -f volumes.yaml

    pod "vol" created

Two containers used one volume — test.<br />
...
$ kubectl exec -ti vol -c busy -- touch /busy/busy<br />
$ kubectl exec -ti vol -c busy -- ls -la /busy
...
Persistent Volumes are a storage abstraction, which provides a standard volume type for Pod: Claims. You define PersistentVolumes Objects backed by an underlying storage provider. Pods mount volumes based on claims they make on the persistent storage.<br />
https://github.com/sebgoa/oreilly-kubernetes/blob/master/manifests/volumes/pvc.yaml<br />
...
PV volume created automatically.<br />
$ kubectl get pv
...
It's inside minikibe.<br />

https://github.com/sebgoa/oreilly-kubernetes/blob/master/manifests/volumes/mysql.yaml<br />
MySQL using this pv.<br />
...
So PV doesn't grow automatic.*<br />
$ kubectl top node
...
# Helm
is the first package manager for kubernetes.<br />
Pic.4.<br />
$ helm create oreilly
...
$ helm install stable/redis<br />
$ REDIS_PASSWORD=$(kubectl get secret --namespace default hissing-manta-redis -o jsonpath="{.data.redis-password}" | base64 --decode)<br />
$ kubectl run hissing-manta-redis-client --rm --tty -i     --env REDIS_PASSWORD=$REDIS_PASSWORD  --image bitnami/redis:3.2.9-r2 -- bash<br />
$ kubectl get pods
...
$ curl https://kubernetes-charts.storage.googleapis.com/index.yaml -o index.yaml<br />
index.yaml search redis:<br />
  `engine: gotpl` # type of template<br />
  and others<br />
...
Redis chart: https://github.com/kubernetes/charts/tree/master/stable/redis<br />
https://github.com/kubernetes/charts/tree/master/stable/redis/templates — manifest for make up the package<br />
https://github.com/kubernetes/charts/blob/master/stable/redis/values.yaml — .Values for templates

## Build your own package
$ cd oreilly-kubernetes/manifests<br />
$ mkdir chart && cd chart<br />
...
This pod have trouble w/ memory, comment limits in velocity/values.yaml and recreate:<br />
$ kubectl get pods
...
Depences resolv into https://github.com/kubernetes/charts/blob/master/stable/joomla/requirements.yaml — requirements.yaml<br />
$ kubectl get deployment
...
*How many replicas changed 3 or 10?*<br />

$ helm upgrade worn-sponge --set image.tag=ghost:0.17 ./velocity/<br />
$ kubectl get pods | grep velocity
...
Examples U might find here:<br />
  https://github.com/kubernetes-incubator/client-python/blob/master/examples/create_deployment.py<br />
  https://github.com/sebgoa/oreilly-kubernetes/blob/master/scripts/create_pod.py

# Custom Resource Defintions
Pic.5.<br />
Kubernetes lets you add your own API objects. Kubernetes can create a new custom API endpoint and provide CRUD operations as well as watch API.<br />
...
Create DB:<br />
  https://github.com/sebgoa/oreilly-kubernetes/blob/master/manifests/crd/db.yml<br />
...
Out API available throught proxy:<br />
$ curl 127.0.0.1:8001/apis/foo.bar/v1/databases<br />
...
Apply HTTP triggered functions from<br />
...
$ kubectl get functions post-python -o yaml<br />
...
# Security
*Namespaces do not give network isolations*, they just give you naming isolation for resources.<br />
By default kubelet don't run privileged container and using priveleged is dangerous.<br />
*Don't pus secrets in secrets, because base64 only included.*<br />
...
Example:<br />
https://github.com/sebgoa/oreilly-kubernetes/blob/master/manifests/security/test.yaml<br />
...
95% containers runs as the root user, and it's too.<br />

-> edit _security/test.yaml_ like a file:
...
Detail documentation about PSP (poe security policies):<br />
  https://docs.bitnami.com/kubernetes/how-to/secure-kubernetes-cluster-psp/

# RBAC (roll based access control)
Quota include in admisstion control (e.g. PSP).<br />
Pic.7.<br />

$ minikube stop<br />
$ minikube start --extra-config=apiserver.Authorization.Mode=RBAC<br />

$ openssl genrsa -out employee.key 2048<br />
$ openssl req -new -key employee.key -out employee.csr -subj "/CN=employee/O=bitnami"<br />
$ export CA_LOCATION=~/.minikube/<br />
$ openssl x509 -req -in employee.csr -CA ${CA_LOCATION}/ca.crt -CAkey ${CA_LOCATION}/ca.key -CAcreateserial -out employee.crt -days 500<br />
...
Error from server (Forbidden): User "employee" cannot list pods in the namespace "office". (get pods)<br />
...
# Sealed Secrets
https://github.com/bitnami/sealed-secrets<br />
...
Run sealed-secret:<br />
$ release=`curl --silent "https://api.github.com/repos/bitnami/sealed-secrets/releases/latest" | sed -n 's/.*"tag_name": *"\([^"]*\)".*/\1/p')])"')`<br />
$ kubectl create -f https://github.com/bitnami/sealed-secrets/releases/download/$release/controller.yaml<br />
...
$ kubeseal <secret.json >mysealedsecret.json<br />
...
Simple python sched:<br />
  https://github.com/sebgoa/oreilly-kubernetes/blob/master/manifests/scheduling/scheduler.py

# Monitoring
https://github.com/sebgoa/oreilly-kubernetes/tree/master/monitoring<br />

Prometheus:<br />
$ kubectl apply -f monitoring-namespace.yaml<br />
$ kubectl apply -f prometheus-rbac.yaml<br />
$ kubectl apply -f prometheus-config.yaml<br />
$ kubectl apply -f prometheus-statefulset.yaml<br />
$ kubectl apply -f prometheus-svc.yaml<br />
$ kubectl apply -f node-exporter-daemonset.yaml<br />
$ kubectl apply -f node-exporter-svc.yam<br />

Grafana:<br />
$ kubectl create -f grafana-statefulset.yaml<br />
$ kubectl create -f grafana-svc.yaml<br />

Pic.8.<br />
Cadviser collect metrics:<br />
  http://192.168.99.100:4194/containers/<br />
$ minikube addons list | grep -i heapster

    - heapster: enabled

# Upgrade
  https://kubernetes.io/docs/tasks/administer-cluster/cluster-management/#upgrading-a-cluster<br />

to make node unscailable/unschedulable:<br />
$ kubectl drain --help<br />
and revert this:<br />
$ kubectl uncordon --help<br />